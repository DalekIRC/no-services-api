...
class Account {
	...
	public static function drop($account, &$error)
	{
		error_log($account);
		$user = new Account($account);
		if (!$user->user) // shouldn't happen since the user must be logged in to do this in the first place
		{
			$error = "User does not exist";
			return 0;
		} else {
			$conn = sqlnew();
			$stmt = $conn->prepare("DELETE FROM userv_account WHERE LOWER(account_name) = :name");
			$stmt->execute(["name" => strtolower($account)]);
			$stmt1 = $conn->prepare("DELETE FROM userv_account_meta WHERE LOWER(user_id) = :name");
			$stmt1->execute(["name" => strtolower($account)]);
			return $stmt->rowCount();
		}
	}
